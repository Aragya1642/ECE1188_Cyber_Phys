...
// Measure and return distance from the Left sensor (Channel 0)
uint32_t OPT3101_GetLeft(void){
  uint32_t distances[3] = {0, 0, 0};
  uint32_t amplitudes[3] = {0, 0, 0};

  OPT3101_StartMeasurementChannel(0);
  while(OPT3101_CheckDistanceSensor() == false){}; // Wait for data ready
  OPT3101_GetMeasurement(distances, amplitudes);

  return distances[0];
}

// Measure and return distance from the Center sensor (Channel 1)
...
// Measure and return distance from the Right sensor (Channel 2)
uint32_t OPT3101_GetRight(void){
  uint32_t distances[3] = {0, 0, 0};
  uint32_t amplitudes[3] = {0, 0, 0};

  OPT3101_StartMeasurementChannel(2);
  while(OPT3101_CheckDistanceSensor() == false){}; // Wait for data ready
  OPT3101_GetMeasurement(distances, amplitudes);
